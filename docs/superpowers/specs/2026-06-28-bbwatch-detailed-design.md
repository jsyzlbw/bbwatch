...
> 本章为 bbwatch 工程设计文档的开篇,定义"我们在解决什么问题、解决到什么程度、不碰什么、用什么词、按什么红线行事、怎样判定成功"。所有涉及 Blackboard(下称 BB)的断言均对应本人账号(CUHK-SZ,userId `_10000_1`)的只读实测,事实依据见 `/private/tmp/.../scratchpad/bb_findings.md`,既有设计稿见 `/Users/mac/Programming/cuhkszbb/docs/superpowers/specs/2026-06-28-bbwatch-design.md`。
...
| 术语 | 定义 | 数据来源(实测字段/端点) |
|---|---|---|
| **uid** | 当前用户的 BB 内部 id,形如 `_10000_1`。`me` 别名在 `users/{uid}/courses` 子资源**不可用**,必须用真实 uid。 | `GET /learn/api/public/v1/users/me` → `id` |
...
class BBClient:
    ...
    def whoami(self) -> str:                                 # GET users/me → id (如 "_10000_1")
...
来源：`GET users/{uid}/courses?expand=course&limit=100`（需翻页；`me` 别名在此子资源**不可用**，必须真实 uid 如 `_10000_1`；uid 本身由 `GET users/me` 解析，findings §1 / 附录 A）。实测 **19 门 membership、17 门在读**。
...
CREATE TABLE IF NOT EXISTS column_status (
    ...
    user_id     TEXT NOT NULL,               -- BB uid, 如 "_10000_1"
...
| key | 含义 | 示例 value |
|---|---|---|
| `bb_user_id` | 真实 uid（courses 子资源必需，由 `users/me` 解析后缓存，findings §1） | `"_10000_1"` |
...
> 课程列表本身也要分页:`users/{uid}/courses?expand=course` 的 `limit≤100`,需翻页(§1 实测)。且**此子资源不接受 `me` 别名**——必须先 `GET users/me` 取真实 `uid`(如 `_10000_1`)再代入。本学期"在读"过滤 = `courseRoleId=Student` 且 `availability.available ∈ {Yes, Term}`(实测 17 门在读),再叠加课程黑/白名单。漏掉一门在读课 = 漏掉它的全部任务,故课程枚举本身也是 $V_k$ 完整性的一环。
...
- 账号形态对齐实测：登录走 ADFS，`UserName` 字段接受 `学号@link.cuhk.edu.cn` 或域账号形式；**`username` 不等于 BB userId**（如 `_10000_1`）。userId 是登录后从 `users/me` 取回并缓存的另一码事（见 6.3 Step 5 / 6.5），二者切勿混用——`users/{uid}/courses` 子资源只认 userId，不认 `me`、更不认登录账号。
...
def adfs_login(curl: cffi.Session, cred: Credential) -> str:
    ...
    返回从 users/me 探针取回的真实 uid (如 '_10000_1')。
...
if r5.status_code == 200 and r5.json().get("id"):
    return r5.json()["id"]                    # 会话就绪, 返回 _10000_1 形态 uid
...
class CachedSession:
    ...
    uid: str                   # _10000_1, 登录后从 users/me 取
...
CREATE TABLE IF NOT EXISTS auth_state (
    ...
    uid              TEXT,                 -- 真实 BB userId, 如 '_10000_1'
...
设计准则贯穿全章：**会话获取与失效自愈封死在 `engine.auth` 内**，`client_id`/`redirect_uri`/ADFS 表单字段全部对齐实测；BB userId（`_10000_1`）由登录探针取定并贯穿，绝不与登录账号或 `me` 别名混用；凭据只活在钥匙串与进程内存、绝不落盘明文/进日志，会话态原子写入 0600 文件；传输层用 curl_cffi 浏览器指纹 + 代理绕开实测 TLS 坑并保留子进程 curl 回退；错误分类驱动"重试/回退/停手/转交互"四种确定性处置；持久化熔断防账号锁定。最关键的是：**会话失效绝不被静默吞成空集**——请求级重放兜底、失败显式抛错、扫描器据此不更新快照，使全量 diff 在下一轮无损补齐，从认证层为"绝不漏 / 绝不重复"上了锁。
...
class BBClient:
    def __init__(self, session: "engine.auth.Session",
        ...
        self.uid: str = self._t.get_json("/users/me",
                                         params={"fields": "id"})["id"]  # e.g. "_10000_1"
...
scan(trigger)                       # trigger ∈ {session_start, manual, periodic}
  ...
  ├─ uid     = resolve_uid(session)            # GET users/me 取真实 _10000_1，缓存
...
来源：`GET /learn/api/public/v1/users/{uid}/courses?expand=course&limit=100`。两个实测约束必须遵守：**(a)** `me` 别名在 `users/me` 可用（用来取 uid），但在 `users/{uid}/courses` 这个**子资源上不可用**，必须传真实 uid（如 `_10000_1`）；**(b)** `limit≤100`，须按 `paging.nextPage` 翻页。配合 `GET /terms?limit=100` 求当前学期。
...
3. 用 `GET users/me` 取真实 uid(形如 `_10000_1`)并缓存——后续 `users/{uid}/courses` 必须用真实 uid(`me` 别名在该子资源不可用)。
...
CREATE TABLE IF NOT EXISTS task (
    ...
    user_id        TEXT NOT NULL,            -- BB userId,如 '_10000_1'
...
> `current_uid()` 不取自请求(防越权指定他人 uid),而由服务启动时从 store 读定的本机单用户 uid(如 `_10000_1`)。本服务单用户,uid 固定。
...
**为什么 uid / term 进状态层而非配置层**：BB userId（实测形如 `_10000_1`）由登录后 `GET /learn/api/public/v1/users/me` 取回，是派生数据而非用户偏好；且 `users/{uid}/courses` 子资源**不接受 `me` 别名、必须用真实 uid**（findings §1），故权威值缓存在 DB `kv` 表（§12.6），随每次登录刷新。配置层只保留一份**只读镜像**供人排查，引擎运行时一律以 DB `kv` 为准、不读配置镜像，避免两处漂移。
...
cached_user_id = ""               # users/me 的 id，如 _10000_1
...
-- 典型 key: 'user_id'(_10000_1) / 'term_id'(2550UG) / 'dashboard_port_active'(8771)
...
[2] 验证登录（最关键的早失败点）
    ...
    - 成功：GET /learn/api/public/v1/users/me 取 id(如 _10000_1) -> 写 DB kv['user_id']
...
**录制方式**:一次性用真实会话抓取各端点响应,经脱敏脚本(替换学号 `120000000`/姓名 `示例同学`/uid `_10000_1`、剥离 cookie/token、保留结构与字段名)落为 fixtures。每个文件对应一个实测端点:

```
tests/contract/fixtures/
  users_me.json                  # users/me → id=_10000_1 (uid 来源)
...
- `test_uid_required_not_me`:断言 bbclient 拉 courses 时 URL 用真实 uid `_10000_1`(由 `users/me` 先取得)而非 `me`(实测 `me` 在该子资源不可用)。
...
def test_paging_concatenates_all_pages(fake_client):
    ...
    got = list(client.list_memberships("_10000_1"))
...
def test_paging_stops_when_no_nextpage(fake_client):
    ...
    assert len(list(client.list_memberships("_10000_1"))) == 1  # 不无限翻页
```
...
另有插件壳层的冒烟测试(不触网):`SessionStart` 钩子脚本以 `--dry-run` 跑通,断言其为 **async 非阻塞**(进程立即返回、扫描在后台)、`additionalContext` 输出含待办摘要,且**不含任何凭据字符串**——grep `Password`、cookie、token、学号 `120000000`、姓名 `示例同学` 必须全为空,对齐"不进日志/不落盘明文"。
...
| 里程碑 | 三刀 | 交付物 | 验收标准 | 依赖 |
|---|---|---|---|---|
| **M0 骨架与传输** | 前置 | repo 结构、`pyproject`、`engine.auth` + `bbclient` 传输层(`curl_cffi`,回退子进程 `curl`,尊重 `127.0.0.1:7890` 代理)、契约 fixtures 录制+脱敏脚本 | 用真实账号一次性跑通 ADFS OAuth2 登录拿到会话 cookie,`GET users/me` 返回 `_10000_1`,翻页拉到 19 门 membership;`requests` TLS 失败已被 `curl_cffi` 规避;凭据只进钥匙串、脱敏脚本输出 grep 凭据为空 | — |
...
4. 凭据 grep(`Password`、cookie、token、学号 `120000000`、姓名 `示例同学`)在源码、日志样本、`additionalContext` 输出、脱敏后的 fixtures 中均为空;
